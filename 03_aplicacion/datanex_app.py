...
I18N = {
    "es": {
        ...
        "show_less": "Ver menos",
        "skills_summary_line": "Tu CV tiene {detected} skills del rol. La ruta recomendada cubre {covered} de los {missing} que te faltan. Solo {remaining} requieren experiencia práctica.",
    ...
    "en": {
        ...
        "show_less": "Show less",
        "skills_summary_line": "Your CV has {detected} skills for this role. The recommended path covers {covered} of the {missing} you are missing. Only {remaining} require hands-on experience.",
...
def read_uploaded_cv(uploaded_file, model_module) -> str:
    ...
    if suffix == ".pdf":
        # Try all parsers + OCR directly from bytes (no disk write needed)
        text = _extract_pdf_from_bytes(file_bytes)
        if text:
            return re.sub(r"\s+", " ", text).strip()
        # Final fallback: write to /tmp and use model module reader
        import tempfile
        try:
...
def render_role_fit_notice(rec: dict, target_role: str, role_label: str):
    ...
    if (
        ...
        if lang == "es":
            ...
            greeting = f"{first_name}, hemos" if first_name else "Hemos"
            line1 = f"{greeting} analizado tu CV y detectamos que tu perfil actual encaja mejor con:"
            line2 = f"Si quieres un camino más corto, considera apuntar primero a <strong style='color:#0AF0C8;'>{escape(best_label)}</strong>. O continúa con <strong style='color:rgba(255,255,255,0.7);'>{escape(role_label)}</strong> — la ruta que te proponemos te llevará ahí."
        else:
            candidate_name = st.session_state.get("caiq_candidate_name", "")
            first_name = candidate_name.split()[0] if candidate_name else ""
            greeting = f"{first_name}, we" if first_name else "We"
            line1 = f"{greeting} analyzed your CV and your current profile fits best with:"
            line2 = f"For a shorter path, consider targeting <strong style='color:#0AF0C8;'>{escape(best_label)}</strong> first. Or keep going with <strong style='color:rgba(255,255,255,0.7);'>{escape(role_label)}</strong> — our recommended path will get you there."

        st.markdown(f"""
        <div style="
          background: rgba(61,110,255,0.06);
          border: 1px solid rgba(61,110,255,0.20);
        ...
        ">
          <div style="font-size:12px;letter-spacing:0.12em;text-transform:uppercase;
            ...
            {"Análisis de perfil" if lang == "es" else "Profile analysis"}
          </div>
          <div style="margin-bottom:12px;color:rgba(255,255,255,0.55);font-size:13px;">
            {escape(line1)}
          </div>
          <div style="display:flex;align-items:center;gap:12px;margin-bottom:14px;">
            <div style="flex:1;background:rgba(10,240,200,0.06);border-radius:8px;padding:12px 16px;
              border:1px solid rgba(10,240,200,0.12);">
              <div style="display:flex;align-items:center;gap:12px;">
                <div>
                  <div style="font-size:22px;font-weight:800;color:#0AF0C8;
                    letter-spacing:-0.02em;line-height:1;">{best_score}%</div>
                  <div style="font-size:10px;color:rgba(10,240,200,0.6);
                    letter-spacing:0.08em;margin-top:2px;">
                    {"compatibilidad" if lang == "es" else "match"}
                  </div>
                </div>
                <div>
                  ...
                  <div style="font-size:11px;color:rgba(255,255,255,0.4);margin-top:2px;">
                    {"tu rol más cercano ahora" if lang == "es" else "your closest current role"}
                  </div>
                </div>
              </div>
            </div>
            <div style="color:rgba(255,255,255,0.2);font-size:18px;">→</div>
            <div style="flex:1;background:rgba(61,110,255,0.06);border-radius:8px;padding:12px 16px;
              border:1px solid rgba(61,110,255,0.15);">
              <div style="font-size:22px;font-weight:800;color:#6B9AFF;
                letter-spacing:-0.02em;line-height:1;">{target_score}%</div>
              <div style="font-size:10px;color:rgba(107,154,255,0.6);
                letter-spacing:0.08em;margin-top:2px;">
                {"compatibilidad" if lang == "es" else "match"}
              </div>
              <div style="font-size:13px;font-weight:600;color:rgba(255,255,255,0.6);margin-top:6px;">
                {escape(role_label)}
              </div>
              <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:2px;">
                {"tu objetivo" if lang == "es" else "your goal"}
              </div>
            </div>
          </div>
...
def render_pdf_export_button(
    ...
    detected_role = str(candidate_profile.get("detected_best_role", "") or candidate_profile.get("detected_role", "") or "")
    strengths = sorted(candidate_skills_explicit)[:6]
    ...
    def chips(skills: list[str], color: str) -> str:
        return "".join(
            f'<span style="display:inline-block;margin:2px;padding:4px 10px;border-radius:100px;'
            f'font-size:11px;font-weight:600;background:{color}20;border:1px solid {color}40;'
            f'color:{color};">{escape(humanize_label(s))}</span>'
            for s in skills[:15]
        )
    ...
    for role, score in sorted(role_scores.items(), key=lambda x: -x[1]):
        ...
        color = "#0AF0C8" if is_target else "#3D6EFF"
        tag = (" ← tu objetivo" if lang == "es" else " ← your goal") if is_target else ""
        bars_html += f"""
        <div style="margin-bottom:10px;">
          <div style="display:flex;justify-content:space-between;font-size:11px;color:#94a3b8;margin-bottom:3px;">
            <span>{escape(label)}{escape(tag)}</span>
            <span style="color:{color};font-weight:700;">{int(score)}%</span>
          </div>
    ...
    if candidate_name and lang == "es":
        title = f"Informe de Carrera CAIQ — {candidate_name}"
    elif candidate_name:
        title = f"CAIQ Career Report — {candidate_name}"
    elif lang == "es":
        title = "Informe de Carrera CAIQ"
    else:
        title = "CAIQ Career Report"
    ...
    strengths_text = ", ".join(escape(humanize_label(s)) for s in strengths) or "—"
    gap_text = ", ".join(escape(humanize_label(s)) for s in priority_gap[:4]) or "—"
    generating_label = "Generando PDF..." if lang == "es" else "Generating PDF..."
    downloaded_label = "✓ PDF descargado" if lang == "es" else "✓ PDF downloaded"
    if lang == "es":
        filename = f"CAIQ_Informe_{safe_name}.pdf" if candidate_name else "CAIQ_Informe_Carrera.pdf"
    else:
        filename = f"CAIQ_Report_{safe_name}.pdf" if candidate_name else "CAIQ_Career_Report.pdf"
...
<style>
  ...
  #download-btn {{
    background: rgba(10,240,200,0.1);
  ...
  #download-btn:hover {{
    background: rgba(10,240,200,0.18);
  }}
  #pdf-content {{
    display: none;
    font-family: 'Segoe UI', Arial, sans-serif;
...
<div id="pdf-content">
  <div class="pdf-header">
    <div>
      <div class="pdf-logo">CAI<span>Q</span></div>
      <div style="font-size:11px;color:#64748b;margin-top:4px;">
        Career Alignment and Insight Qualifier
      </div>
    </div>
    <div style="text-align:right;">
      <div style="font-size:14px;font-weight:700;color:#fff;">{escape(title)}</div>
      <div class="pdf-date">{escape(today)}</div>
    </div>
  </div>

  <div class="section-title">{"Resumen del perfil" if lang == "es" else "Profile summary"}</div>
  <div class="profile-grid">
    <div class="profile-card">
      <div class="profile-card-label">{"Perfil detectado" if lang == "es" else "Detected profile"}</div>
      <div class="profile-card-value">{escape(detected_label)}</div>
    </div>
    <div class="profile-card">
      <div class="profile-card-label">{"Rol objetivo" if lang == "es" else "Target role"}</div>
      <div class="profile-card-value">{escape(role_label)}</div>
    </div>
    <div class="profile-card">
      <div class="profile-card-label">{"Fortalezas" if lang == "es" else "Strengths"}</div>
      <div class="profile-card-value" style="font-size:12px;">{strengths_text}</div>
    </div>
    <div class="profile-card">
      <div class="profile-card-label">{"Gap principal" if lang == "es" else "Main gap"}</div>
      <div class="profile-card-value" style="font-size:12px;">{gap_text}</div>
    </div>
  </div>

  <div class="section-title">{"Métricas" if lang == "es" else "Metrics"}</div>
  <div class="metrics-row" style="grid-template-columns:1fr 1fr;">
    <div class="metric-card">
      <div class="metric-num" style="color:#0AF0C8;">{skills_n}</div>
      <div class="metric-label">{"Skills en tu CV" if lang == "es" else "Skills in CV"}</div>
    </div>
    <div class="metric-card">
      <div class="metric-num" style="color:#FF6B85;">{gap_n}</div>
      <div class="metric-label">{"Skills que te faltan" if lang == "es" else "Missing skills"}</div>
    </div>
  </div>
  <div class="progress-label">
    <span>{"La ruta cubre" if lang == "es" else "Path covers"}</span>
    <span style="color:#0AF0C8;font-weight:600;">{covered_n} {"de" if lang == "es" else "of"} {gap_n} skills ({pct}%)</span>
  </div>
  <div class="progress-track">
    <div class="progress-fill" style="width:{pct}%;"></div>
  </div>

  <div class="section-title">{"Compatibilidad con cada rol" if lang == "es" else "Compatibility per role"}</div>
  <div style="background:#1e293b;border-radius:10px;padding:16px 20px;margin-bottom:8px;">
    {bars_html}
  </div>

  <div class="section-title">{"Skills detectadas" if lang == "es" else "Detected skills"}</div>
  <div style="margin-bottom:8px;">
    <div style="font-size:11px;color:#64748b;margin-bottom:6px;">{"Ya tienes" if lang == "es" else "You have"}</div>
    {chips(sorted(candidate_skills_explicit), "#0AF0C8")}
  </div>
  <div style="margin-bottom:8px;margin-top:14px;">
    <div style="font-size:11px;color:#64748b;margin-bottom:6px;">{"Te faltan" if lang == "es" else "Missing"}</div>
    {chips(priority_gap, "#FF6B85")}
  </div>
  <div style="margin-bottom:8px;margin-top:14px;">
    <div style="font-size:11px;color:#64748b;margin-bottom:6px;">{"La ruta cubre" if lang == "es" else "Path covers"}</div>
    {chips(covered, "#6B9AFF")}
  </div>

  <div class="footer">CAIQ — Career Alignment and Insight Qualifier · caiq.app · {escape(today)}</div>
</div>

<script>
function generatePDF() {{
  var btn = document.getElementById('download-btn');
  btn.textContent = {json.dumps(generating_label)};
  btn.disabled = true;
  ...
  html2pdf().set(opt).from(content).save().then(function() {{
    ...
    btn.textContent = {json.dumps(downloaded_label)};
    btn.style.color = '#0AF0C8';
    setTimeout(function() {{
      btn.textContent = {json.dumps(btn_label)};
      btn.disabled = false;
      btn.style.color = '#0AF0C8';
    }}, 3000);
  }}).catch(function() {{
...
def assess_job_readiness(candidate_profile: dict, target_role: str) -> dict:
    ...
    strong_skills = {
        "python", "sql", "r", "power_bi", "tableau", "excel", "statistics",
        "machine_learning", "deep_learning", "etl", "airflow", "spark", "dbt",
        "aws", "azure", "gcp", "pandas", "numpy", "scikit_learn",
        "data_engineering", "data_visualization", "forecasting", "experimentation",
        "databricks", "docker", "kubernetes", "api",
    }
    abstract_skills = {
    ...
    if (not eligible) and stage == "bridge" and analyst_family and len(technical_verified) >= 2:
        eligible = True
    # Soft unlock: if candidate has ≥3 strong technical skills (even just declared)
    # and a reasonable readiness score, show jobs as aspirational targets.
    if (not eligible) and len(technical_detected) >= 3 and readiness_score >= 0.28:
        eligible = True
...
def render_skill_gap_radar(
    ...
    """Radar chart: perfil actual vs rol objetivo vs tras la ruta de aprendizaje."""
    lang = st.session_state.get("lang", "es")
    ...
    # Top skills del rol objetivo por demanda
    ...
    top_skills = list(role_df["skill"].dropna().astype(str).str.lower().unique()[:max_skills])
    if not top_skills:
        return

    candidate_norm = {str(s).strip().lower() for s in candidate_skills_explicit}
    gap_set = {str(s).strip().lower() for s in rec.get("gap_skills", [])}
    remaining = {str(s).strip().lower() for s in rec.get("remaining_gap", [])}
    covered_by_path = gap_set - remaining  # skills que la ruta cubre

    # Valores por skill: 1.0 = tiene, 0.55 = cubierto por ruta, 0.12 = falta
    vals_current, vals_after = [], []
    for skill in top_skills:
        has_it = skill in candidate_norm
        covered = (not has_it) and (skill in covered_by_path)
        vals_current.append(1.0 if has_it else 0.12)
        vals_after.append(1.0 if (has_it or covered) else 0.12)

    labels = [s.replace("_", " ").title() for s in top_skills]
    # Cerrar el polígono
    labels_closed = labels + [labels[0]]
    vals_current_closed = vals_current + [vals_current[0]]
    vals_after_closed = vals_after + [vals_after[0]]

    fig = go.Figure()

    # Trace 1: tras la ruta
    fig.add_trace(go.Scatterpolar(
        r=vals_after_closed,
        theta=labels_closed,
        fill="toself",
        fillcolor="rgba(10,240,200,0.12)",
        line=dict(color="rgba(10,240,200,0.5)", width=1.5, dash="dot"),
        name="Tras la ruta" if lang == "es" else "After path",
        hovertemplate="%{theta}: cubierto tras la ruta<extra></extra>",
    ))

    # Trace 2: perfil actual (encima)
    fig.add_trace(go.Scatterpolar(
        r=vals_current_closed,
        theta=labels_closed,
        fill="toself",
        fillcolor="rgba(139,92,246,0.25)",
        line=dict(color="#8B5CF6", width=2),
        name="Tu perfil" if lang == "es" else "Your profile",
    ...
    fig.update_layout(
        polar=dict(
            ...
            angularaxis=dict(
                ...
                tickfont=dict(size=11, color="rgba(255,255,255,0.75)"),
            ),
            radialaxis=dict(
                visible=False,
                range=[0, 1],
    ...
    title = "Skill Gap — Perfil vs Rol objetivo" if lang == "es" else "Skill Gap — Profile vs Target Role"
    st.markdown(
        f'<div style="font-size:10px;letter-spacing:0.18em;text-transform:uppercase;'
        f'color:rgba(10,240,200,0.6);margin:20px 0 4px;">{title}</div>',
        unsafe_allow_html=True,
    )
    # Caption: clarify this shows only top-N role skills
    n_shown = len(top_skills)
    cap_es = f"Top {n_shown} skills más demandados para este rol. Verde punteado = perfil tras completar la ruta recomendada."
    cap_en = f"Top {n_shown} most demanded skills for this role. Green dotted line = profile after completing the recommended path."
    st.caption(cap_es if st.session_state.get("lang", "es") == "es" else cap_en)
    st.plotly_chart(fig, use_container_width=True, config={"displayModeBar": False})

    # Leyenda textual debajo del radar — usar vals sin el elemento de cierre (closed polygon)
    n_have    = sum(1 for v in vals_current[:-1] if v > 0.5)
    n_covered = sum(1 for c, a in zip(vals_current[:-1], vals_after[:-1]) if c < 0.5 and a > 0.5)
    n_missing = sum(1 for v in vals_after[:-1] if v < 0.5)
    legend_html = (
        f'<div style="display:flex;gap:20px;font-size:12px;color:rgba(255,255,255,0.55);'
        f'margin-bottom:16px;flex-wrap:wrap;">'
        f'<span><span style="color:#8B5CF6;font-weight:700;">■</span> '
        f'{"Tienes" if lang=="es" else "You have"}: {n_have}</span>'
        f'<span><span style="color:#0AF0C8;font-weight:700;">■</span> '
        f'{"La ruta cubre" if lang=="es" else "Path covers"}: {n_covered}</span>'
        f'<span><span style="color:rgba(255,255,255,0.25);font-weight:700;">■</span> '
        f'{"Aún falta" if lang=="es" else "Still missing"}: {n_missing}</span>'
        f'</div>'
    )
...
def render_skills_intelligence(candidate_skills_explicit: set[str], rec: dict):
    ...
    with st.container():
        ...
        st.caption(
            tr(
                "skills_summary_line",
                detected=len(candidate_skills_explicit),
                missing=len(gap_set),
                covered=len(covered_by_path),
                remaining=len(rec.get("remaining_gap", [])),
            )
        )
        if core_cov >= 0.65:
...
def main():
    ...
    countries = sorted([c for c in location_df["country"].dropna().unique().tolist() if c])
    sidebar_field_label(tr("country"))
    selected_country = st.sidebar.selectbox(tr("country"), [tr("all_countries")] + countries, index=0, label_visibility="collapsed")

    if selected_country == tr("all_countries"):
        locality_values = sorted(location_df["location"].unique().tolist())
    else:
    ...
    if cache_payload and cache_payload.get("key") == reco_cache_key:
        ...
        # Soft unlock: if role fit is already strong, do not fully block jobs.
        # Show realistic/aspirational options even when strict gate is not met.
            can_unlock_with_score = (
                float(rec.get("role_match_score_current", 0.0)
 or 0.0) >= 60.0
            ...
            if readiness.get("eligible", False):
                ...
                jobs_notice_key = "jobs_unlocked_via_path"
            else:
                jobs = pd.DataFrame()
                jobs_notice_key = "jobs_not_yet_eligible"
    ...
    if _cache:
        try:
            ...
            st.error(f"Error al renderizar resultados: {_render_err}")
